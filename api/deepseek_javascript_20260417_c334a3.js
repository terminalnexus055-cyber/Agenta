...
// Upgrades v4: all Groq calls use llama-3.1-8b-instant (30k TPM) + reduced tokens
...
// ─── GROQ CALL WITH RETRY + BACKOFF (model default changed to 8b) ───
async function callGroq(systemPrompt, userPrompt, maxTokens = 1500, model = 'llama-3.1-8b-instant', retries = 3) {
  let lastError;
  for (let attempt = 0; attempt < retries; attempt++) {
    ...
    try {
      ...
      if (res.status === 429) {
        ...
        console.warn(`Groq rate limit hit. Waiting ${waitSec}s before retry ${attempt+1}/${retries}`);
      ...
      console.warn(`Groq call failed: ${e.message}. Retrying in ${backoff}ms...`);
...
async function callGemini(systemPrompt, userPrompt, maxTokens = 800) {
  if (!GEMINI_API_KEY) {
    throw new Error('GEMINI_API_KEY is required for Architect. No fallback to Groq.');
  }
...
// ─── IDENTIFY TARGET FILES (unchanged) ───
...
// ─── BUILD CONTEXT STRING (reduced to 5000 chars) ───
...
${repoMemory.recentPatterns.map(r =>
  ...
  return parts.join('\n\n').slice(0, 5000);  // reduced from 8000
}
...
// ─── MAIN HANDLER (with reduced maxTokens) ───
export default async function handler(req, res) {
  ...
  try {
    ...
    // STEP 2: Coder (Groq) - using 8b model, reduced maxTokens
    const coderOutput = await callGroq(
      prompts.coder,
      `CODEBASE CONTEXT:\n${contextStr}\n\nARCHITECT'S PLAN:\n${architectPlan}\n\nORIGINAL TASK:\n${prompt}\n\nWrite the complete implementation now.`,
      1200,  // reduced from 1800
      'llama-3.1-8b-instant'
    );

    // STEP 3: Reviewer (Groq, 8b model)
