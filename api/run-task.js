...
export default async function handler(req, res) {
  ...
  try {
    ...
    const hasIssues = reviewerOutput.toLowerCase().includes('confidence: low') ||
                      reviewerOutput.toLowerCase().includes('critical') ||
                      reviewerOutput.toLowerCase().includes('bug found') ||
                      reviewerOutput.toLowerCase().includes('corrected version');

    if (hasIssues) {
      // Extract corrected code from reviewer if present
      const correctedMatch = reviewerOutput.match(/corrected version[\s\S]*?```[\s\S]*?```/i);
      if (correctedMatch) {
        finalOutput = `[Reviewer corrected the implementation]\n\n${reviewerOutput}`;
      } else {
        // Ask Groq to fix based on review feedback
        const fixPrompt = `ORIGINAL IMPLEMENTATION:\n${coderOutput}\n\nREVIEWER FEEDBACK:\n${reviewerOutput}\n\nFix the issues identified by the reviewer. Output the corrected code only.`;
        finalOutput = await callGroq(prompts.coder, fixPrompt, 2000);
      }
    }
